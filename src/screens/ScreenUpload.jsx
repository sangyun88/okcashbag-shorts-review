    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: T.white, overflowY: 'auto' }}>
          ...
          {STEPS.map((step, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
              ...
              <div style={{
                ...
                background: i <= uploadStep ? T.ocbRed : T.gray200,
              ...
              <span style={{ fontSize: 10, color: i === uploadStep ? T.ocbRed : T.gray400, marginTop: 4, fontWeight: i === uploadStep ? 700 : 500, textAlign: 'center' }}>{step}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {uploadStep === 0 && (
        <div style={{ flex: 1, padding: '22px 16px' }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: T.gray800, marginBottom: 5, letterSpacing: -0.4 }}>리뷰를 작성할 상품을 선택하세요</h2>
          <p style={{ fontSize: 13, color: T.gray400, fontWeight: 500, marginBottom: 20 }}>구매한 상품에 한해 숏츠리뷰를 작성할 수 있어요</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {MY_ORDERS.map((o) => (
              <button key={o.id} onClick={() => !o.hasReview && setSelectedOrder(o.id)} style={{
                display: 'flex', alignItems: 'center', gap: 12, padding: '14px',
                background: selectedOrder === o.id ? '#fff5f5' : T.gray50,
                border: selectedOrder === o.id ? `1.5px solid ${T.ocbRed}` : `1px solid ${T.gray100}`,
                borderRadius: 14, cursor: o.hasReview ? 'default' : 'pointer',
                opacity: o.hasReview ? 0.5 : 1, fontFamily: 'inherit',
                boxShadow: selectedOrder === o.id ? '0 2px 8px rgba(255,90,95,0.15)' : T.shadowCard,
              }}>
                <div style={{
                ...
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <p style={{ fontSize: 11, color: T.gray400, fontWeight: 500, margin: 0 }}>{o.brand} · {o.date}</p>
                  <p style={{ fontSize: 13, fontWeight: 600, color: T.gray800, margin: '3px 0 5px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', letterSpacing: -0.2 }}>{o.product}</p>
                  {o.hasReview
                    ? <span style={{ fontSize: 11, color: T.gray400, background: T.gray100, borderRadius: 9999, padding: '2px 8px' }}>리뷰 작성 완료</span>
                    : <span style={{ fontSize: 11, color: T.ocbRed, background: '#fff0f0', borderRadius: 9999, padding: '2px 8px', fontWeight: 700 }}>+50P 지급 가능</span>
                  }
                </div>
                {selectedOrder === o.id && (
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
          ...
          <button onClick={() => selectedOrder && setUploadStep(1)} style={{
            ...
            fontFamily: 'inherit', marginTop: 24, letterSpacing: -0.2,
          }}>다음 단계</button>
        </div>
      )}
      ...
      {uploadStep === 1 && (
        <div style={{ flex: 1, padding: '22px 16px' }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: T.gray800, marginBottom: 5, letterSpacing: -0.4 }}>영상을 선택하고 편집하세요</h2>
          <p style={{ fontSize: 13, color: T.gray400, fontWeight: 500, marginBottom: 20 }}>최대 5초, 20MB 이하의 영상만 업로드 가능해요</p>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: 15, fontWeight: 600, color: T.gray700, margin: 0, letterSpacing: -0.2 }}>갤러리에서 선택</p>
              <p style={{ fontSize: 12, color: T.gray400, fontWeight: 500, marginTop: 4 }}>또는 카메라로 바로 촬영</p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
            {[
              ...
              <button key={label} style={{
                ...
                <span style={{ fontSize: 12, color: T.gray600, fontWeight: 500 }}>{label}</span>
              </button>
            ))}
          </div>

          <p style={{ fontSize: 12, color: T.gray400, fontWeight: 500, textAlign: 'center', marginBottom: 16 }}>
      ...
      {uploadStep === 2 && (
        <div style={{ flex: 1, padding: '22px 16px' }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: T.gray800, marginBottom: 5, letterSpacing: -0.4 }}>영상에 멘트를 추가하세요</h2>
          <p style={{ fontSize: 13, color: T.gray400, fontWeight: 500, marginBottom: 20 }}>최대 20자, 영상 중앙에 표시됩니다</p>
          ...
          <div style={{
            ...
            <div style={{
              padding: '8px 20px', minWidth: 120, textAlign: 'center',
            }}>
              <span style={{
                color: T.white, fontSize: 18, fontWeight: 500, letterSpacing: -0.3,
              }}>
                {uploadCaption || '멘트를 입력하세요'}
              </span>
            </div>
            <div style={{ position: 'absolute', top: 10, left: 10, background: 'rgba(0,0,0,0.45)', borderRadius: 9999, padding: '3px 10px' }}>
          ...
          <div style={{ marginBottom: 8 }}>
            <div style={{
              ...
              border: filterText ? `1.5px solid ${T.ocbRed}` : `1.5px solid ${T.gray200}`,
            }}>
              <input
                value={uploadCaption}
                ...
                style={{
                  ...
                  fontSize: 16, color: T.gray800, fontFamily: 'inherit', letterSpacing: -0.2,
                }}
              />
            ...
            {filterText && (
              <p style={{ fontSize: 12, color: T.ocbRed, marginTop: 5, marginLeft: 4, fontWeight: 500 }}>⚠ 사용할 수 없는 표현이 포함되어 있어요</p>
            )}
          </div>

          <p style={{ fontSize: 12, color: T.gray400, fontWeight: 500, marginBottom: 20 }}>
          ...
          <button
            ...
            style={{
              width: '100%', padding: '14px', borderRadius: 10, border: 'none',
              background: (!filterText && uploadCaption.trim()) ? T.brandGrad : T.gray200,
