...
export default function ScreenProductDetail() {
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: T.white }} className="scrollbar-hide">
      {/* 상품 이미지 영역 */}
      <div style={{
        height: 256, background: PRODUCT.gradient,
        ...
        <div style={{
          ...
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none">
            <path d="M6 2L3 6V20C3 21.1 3.9 22 5 22H19C20.1 22 21 21.1 21 20V6L18 2H6Z" stroke="rgba(255,255,255,0.75)" strokeWidth="2" strokeLinejoin="round"/>
            <path d="M3 6H21" stroke="rgba(255,255,255,0.75)" strokeWidth="2"/>
          </svg>
        </div>
        <div style={{
          position: 'absolute', top: 10, right: 10,
          background: T.ocbRed, borderRadius: 9999, padding: '3px 10px',
        }}>
          <span style={{ color: T.white, fontSize: 11, fontWeight: 700, letterSpacing: 0.2 }}>{PRODUCT.disc} OFF</span>
        </div>
      </div>

      {/* 상품 정보 */}
      <div style={{ padding: '16px 16px 18px', borderBottom: `1px solid ${T.gray100}` }}>
        <p style={{ fontSize: 11, color: T.gray400, fontWeight: 600, margin: 0, letterSpacing: 0.3, textTransform: 'uppercase' }}>{PRODUCT.brand}</p>
        <p style={{ fontSize: 18, fontWeight: 700, color: T.gray800, margin: '5px 0 10px', lineHeight: 1.3, letterSpacing: -0.4 }}>{PRODUCT.name}</p>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <span style={{ fontSize: 22, fontWeight: 800, color: T.ocbRed, letterSpacing: -0.5 }}>{PRODUCT.price}</span>
          <span style={{ fontSize: 14, color: T.gray300, textDecoration: 'line-through' }}>{PRODUCT.origPrice}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8 }}>
          <div style={{ display: 'flex', gap: 2 }}>
            {[1,2,3,4,5].map((s) => (
              <svg key={s} width="13" height="13" viewBox="0 0 24 24" fill={s <= Math.floor(PRODUCT.rating) ? T.gold : T.gray200}>
        ...
        {['상품정보', '숏츠리뷰', '텍스트리뷰'].map((tab) => {
          ...
          return (
            <button key={tab} onClick={() => setDetailTab(tab)} style={{
              ...
              padding: '13px 0', fontSize: 13, fontWeight: on ? 700 : 500,
              ...
              {isShorts && (
                <svg width="11" height="11" viewBox="0 0 24 24" fill={on ? T.gray800 : T.gray400}>
                  <polygon points="5,3 19,12 5,21"/>
                </svg>
              )}
              {tab}
              {isShorts && (
                <span style={{ background: T.ocbRed, color: T.white, fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 9999 }}>
                  {REVIEWS.length}
                </span>
              )}
            </button>
      ...
      <div style={{
        ...
        <button onClick={() => setScreen('upload')} style={{
          ...
          <svg width="14" height="14" viewBox="0 0 24 24" fill={T.white}>
            <polygon points="5,3 19,12 5,21"/>
          </svg>
          숏츠리뷰 작성 (+50P)
        </button>
  ...
  return (
    <div style={{ padding: '14px' }}>
      ...
      <div style={{
        ...
        background: T.gray50, borderRadius: 14, overflow: 'hidden',
        ...
        {[
          ...
          <div key={label} style={{
            ...
            borderRight: idx < 2 ? `1px solid ${T.gray100}` : 'none',
          }}>
            <p style={{ fontSize: 18, fontWeight: 800, color: T.gray800, margin: 0, letterSpacing: -0.5 }}>{value}{unit}</p>
            <p style={{ fontSize: 11, color: T.gray400, margin: '3px 0 0', fontWeight: 500 }}>{label}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
        {REVIEWS.map((r, i) => (
          <button key={r.id} onClick={() => { setCurrentIndex(i); setScreen('feed'); }} style={{
            ...
            position: 'relative', boxShadow: T.shadowCard,
          }}>
            <div style={{ position: 'absolute', inset: 0, background: r.gradient }} />
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.65) 0%, transparent 100%)', padding: '10px 6px 6px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                <svg width="9" height="9" viewBox="0 0 24 24" fill="rgba(255,255,255,0.85)">
                  <path d="M1 12C1 12 5 4 12 4C19 4 23 12 23 12C23 12 19 20 12 20C5 20 1 12 1 12Z"/>
                  <circle cx="12" cy="12" r="3" fill="rgba(0,0,0,0.3)"/>
                </svg>
                <span style={{ fontSize: 10, color: T.white, fontWeight: 500 }}>{fmtNum(r.views)}</span>
              </div>
            </div>
          </button>
        ))}
...
function TextReviewTab() {
  ...
  return (
    <div style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
      {TEXTS.map((r, i) => (
        <div key={i} style={{
          padding: '14px', background: T.gray50, borderRadius: 14,
          border: `1px solid ${T.gray100}`,
        }}>
...
function ProductInfoTab() {
  return (
    <div style={{ padding: '20px 14px', textAlign: 'center' }}>
      <div style={{
        height: 160, background: T.gray50, borderRadius: 14,
        border: `1px solid ${T.gray100}`,
        display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16,
      }}>
        <span style={{ color: T.gray300, fontSize: 14 }}>상품 상세 이미지</span>
      </div>
      <p style={{ fontSize: 14, color: T.gray600, lineHeight: 1.65 }}>
